...
func a(services []msg.Service, state request.Request) (records []dns.RR) {
	...
	for _, s := range services {
		...
		case dns.TypeA:
			if _, ok := dup[s.Host]; !ok {
				dup[s.Host] = struct{}{}
				records = append(records, s.NewA(state.QName(), ip))
			}
...
func aaaa(services []msg.Service, state request.Request) (records []dns.RR) {
	dup := make(map[string]struct{})
	...
	for _, s := range services {
		...
		case dns.TypeAAAA:
			if _, ok := dup[s.Host]; !ok {
				dup[s.Host] = struct{}{}
				records = append(records, s.NewAAAA(state.QName(), ip))
			}
		}
	}
	return records
...
func srv(services []msg.Service, state request.Request) (records, extra []dns.RR) {
	...
	for _, s := range services {
		...
		case dns.TypeA, dns.TypeAAAA:
			...
			if ok := isDuplicate(dup, srv.Target, addr, 0); !ok {
				hdr := dns.RR_Header{Name: srv.Target, Rrtype: what, Class: dns.ClassINET, Ttl: s.TTL}
